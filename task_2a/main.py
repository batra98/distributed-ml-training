...
def sync_gradients(model: nn.Module):
    """Gathers gradients from all workers, computes mean at rank 0, and scatters it back."""
    world_size = dist.get_world_size()
    if world_size == 1:
        return  # No need for synchronization if only one process
    ...
    for param in model.parameters():
        if param.grad is not None:
            ...
            dist.gather(
                param.grad,
                gather_list=gathered_grads if dist.get_rank() == 0 else None,
                dst=0,
            )

            if dist.get_rank() == 0:
                avg_grad = sum(gathered_grads) / world_size  # Compute average
                for i in range(world_size):
                    gathered_grads[i] = avg_grad  # Assign averaged gradient

            # Scatter the updated gradients back to all workers
            dist.scatter(
                param.grad,
                scatter_list=gathered_grads if dist.get_rank() == 0 else None,
                src=0,
            )
...
def main():
    ...
    writer = (
        SummaryWriter(log_dir=f"../logs_40/task_2a/rank_{args.rank}")
        if args.rank == 0
        else None
    )
